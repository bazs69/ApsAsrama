...
export default function RoleUserClient({ initialRoles, allPermissions }: { initialRoles: Role[], allPermissions: Permission[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.refresh() // Smooth Next.js reload without flashing
    } catch (err) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-white dark:bg-zinc-900/90 text-zinc-900 dark:text-zinc-100 border border-success-100 dark:border-success-900/30 hover:border-success-300 dark:hover:border-success-700/50 p-6 rounded-2xl shadow-sm border border-zinc-200 dark:border-zinc-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {roles.map(role => (
          <div key={role.id} className="bg-white dark:bg-zinc-900/90 text-zinc-900 dark:text-zinc-100 border border-success-100 dark:border-success-900/30 hover:border-success-300 dark:hover:border-success-700/50 rounded-2xl p-6 border border-zinc-200 dark:border-zinc-800 shadow-sm relative overflow-hidden group">
            {role.isSystem && (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white dark:bg-zinc-900/90 text-zinc-900 dark:text-zinc-100 border border-success-100 dark:border-success-900/30 hover:border-success-300 dark:hover:border-success-700/50 rounded-2xl w-full max-w-2xl shadow-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden my-8">
                  ...
                  {Object.entries(permissionTree).map(([mod, perms]) => {
                    ...
                    return (
                      <div key={mod} className="border-b border-zinc-200 dark:border-zinc-800 last:border-0">
                        ...
                        {isExpanded && (
                          <div className="pl-12 pr-4 pb-3 grid grid-cols-2 gap-2 bg-white dark:bg-zinc-900/90 text-zinc-900 dark:text-zinc-100 border border-success-100 dark:border-success-900/30 hover:border-success-300 dark:hover:border-success-700/50">
                            {perms.map(p => (
