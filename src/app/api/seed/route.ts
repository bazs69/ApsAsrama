...
export async function GET() {
  ...
  try {
    ...
    if (existingAdmin) {
      ...
      return NextResponse.json({
        message: "Admin password successfully reset and role assigned!",
        admin: {
    ...
    return NextResponse.json({
      message: "Seeding RBAC completed successfully!",
      admin: {
        id: admin.id,
        name: admin.name,
        email: admin.email,
    ...
    return NextResponse.json(
      { message: "Seeding failed", error: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    )
  }
}
